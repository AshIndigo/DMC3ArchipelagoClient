...
use crate::{constants, hook, tables, utilities};
...
fn install_jump_rax_for_itm_file(custom_function: usize) {
    ...
    unsafe {
        hook::modify_call_offset(0x23ba41usize + utilities::get_dmc3_base_address(), 13); //sub
        hook::modify_call_offset(0x23ce70usize + utilities::get_dmc3_base_address(), 13); //sub fixes key items as well...
...
fn install_jump_rax_for_in_world(custom_function: usize) {
    // This is for Location checking
    unsafe {
        hook::modify_call_offset(0x1b7143usize + utilities::get_dmc3_base_address(), 11); //sub
